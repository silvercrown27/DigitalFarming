...
def custom_image_dataset_from_dir_pdd(train_csv, train_path, test_csv, test_path, image_size=(128, 128),
    ...
    train_labels = encoder.fit_transform(train_labels.reshape(-1, 1)).astype("float32")
    test_labels = encoder.transform(test_labels.reshape(-1, 1)).astype("float32")
    ...
    x_train, x_valid, y_train, y_valid = tr_images, train_labels, ts_images, test_labels

    train_dataset = tf.data.Dataset.from_tensor_slices((x_train, x_valid))
    test_dataset = tf.data.Dataset.from_tensor_slices((y_train, y_valid))

    if shuffle:
        train_dataset = train_dataset.shuffle(len(x_train))
        test_dataset = test_dataset.shuffle(len(x_valid))
